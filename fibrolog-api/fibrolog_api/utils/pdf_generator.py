...
class ReportPDF(BaseDocTemplate):
    def __init__(self, filename, title='Relatório FibroLog', **kw):
        ...
        frame_first = Frame(margin_first, margin_first, A4[0] - 2 * margin_first, A4[1] - 2 * margin_first, id='first')

        # Frame páginas seguintes (margem superior maior)
        frame_later = Frame(margin_later, margin_first, A4[0] - 2 * margin_later, A4[1] - margin_first - margin_later - 0.5 * cm, id='later')

        # Add page templates
        self.addPageTemplates([
            PageTemplate(id='First', frames=frame_first, onPage=self._header_footer),
            PageTemplate(id='Later', frames=frame_later, onPage=self._header_footer)
        ])
...
def render_pain_chart(timeline_data):
    ...
    lp.xValueAxis.labelTextFormat = lambda x: labels[int(x)] if 0 <= int(x) < len(labels) else ''
...
def generate_report_pdf(report_data):
    ...
    # Title
    elements.append(Paragraph("Relatório de Monitoramento – Fibromialgia", title_style))
    elements.append(Paragraph(f"Período: {report_data['period']}", styles['Normal']))
    elements.append(Spacer(1, 0.5 * cm))

    # Patient Info
    elements.append(Paragraph(f"<b>Paciente:</b> {report_data['patientName']}", styles['Normal']))
    elements.append(Paragraph(f"<b>Data de geração:</b> {report_data['generationDate'].strftime('%d/%m/%Y')}", styles['Normal']))
    elements.append(Spacer(1, 0.5 * cm))
    ...
    summary_data = [
        [f"Média de dor diária: {summary['averagePain']}/10", f"Pico de dor mais alto: {summary['peakPain']}/10"],
        [f"Dias com dor intensa (>7): {summary['intensePainDays']}", f"Nível médio de fadiga: {summary['averageFatigue']}/10"],
    ...
    elements.append(Paragraph("<b>Frequência de estados emocionais:</b>", styles['Normal']))

    emotions = summary['emotionFrequency']
    emotion_list = ", ".join([f"{k}: {v} d" for k, v in emotions.items() if v > 0])
    elements.append(Paragraph(emotion_list or "Nenhum registro no período.", styles['Normal']))
    ...
    if chart:
        ...
        elements.append(Paragraph("Dados insuficientes para gerar o gráfico.", styles['Normal']))
    ...
    frequent_list = [f"• Região ID {r['id']}: {r['count']} registros" for r in frequent]
    ...
    layout_data = [
        [
            Paragraph("<br/><br/>".join(frequent_list) or "Nenhuma região relatada.", styles['Normal']),
            drawing
        ]
    ]
    ...
    timeline_header = [["Data", "Dor", "Fad.", "Sono", "Emoção", "Observações"]]
    ...
    timeline_table = Table(timeline_data, colWidths=[1.5 * cm, 1.2 * cm, 1.2 * cm, 1.2 * cm, 2.5 * cm, 9.5 * cm], repeatRows=1)
    ...
    if report_data['crisisHistory']:
        ...
        crisis_header = [["Data/Hora", "Int.", "Dur.", "Sintomas Relatados", "Contexto"]]
        ...
        crisis_table = Table(crisis_data, colWidths=[2.5 * cm, 1.2 * cm, 1.5 * cm, 5.5 * cm, 6.5 * cm], repeatRows=1)
        crisis_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#A52A2A')),  # Dark red for crises
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
