...
MIN_PDF_SIZE = 10000  # Tamanho mínimo esperado do PDF em bytes
...
async def test_gerar_relatorio_pdf_sucesso(client, token, paciente, session):
    ...
    session.add(
        RegistroSintoma(registro_id=r.id, sintoma_id='1', intensidade=8)
    )
    session.add(
        RegistroRegiaoDor(registro_id=r.id, regiao_id='10', intensidade=5)
    )
...
@pytest.mark.asyncio
async def test_gerar_relatorio_pdf_muitas_paginas(
    ...
    for i in range(40):  # 40 days of logs should exceed one page
        ...
        await session.flush()
        session.add(
            RegistroSintoma(registro_id=r.id, sintoma_id='1', intensidade=5)
        )
    ...
    assert len(response.content) > MIN_PDF_SIZE
    assert response.content.startswith(b'%PDF')
